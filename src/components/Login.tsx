...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left brand panel */}
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-14 text-white relative overflow-hidden"
        ...
        <div className="flex items-center gap-2 text-2xl font-black relative z-10">
          <span className="bg-white/20 rounded-xl px-3 py-1">🏠</span> Kotha Pasal
        </div>
        <div className="relative z-10">
          <h1 className="text-5xl font-black leading-tight tracking-tight">Find your<br/>perfect kotha.</h1>
          <p className="mt-4 text-white/85 text-lg max-w-md">Verified rooms & student hostels across all 77 districts of Nepal — direct from owners, zero broker spam.</p>
          <div className="flex gap-3 mt-8">
            {[['Nearby', MapPin], ['Verified', ShieldCheck], ['Instant', Bolt]].map(([t, Icon]: any) => (
              <span key={t} className="inline-flex items-center gap-2 bg-white/15 backdrop-blur px-4 py-2 rounded-full text-sm font-semibold">
                <Icon className="w-4 h-4" /> {t}
              </span>
            ))}
          </div>
        </div>
        <p className="relative z-10 text-white/60 text-sm">© 2026 Kotha Pasal</p>
      </div>

      {/* Right form */}
      <div className="flex-1 flex items-center justify-center p-6 bg-surface-bg">
        <div className="w-full max-w-md">
          <div className="lg:hidden text-2xl font-black text-primary mb-8 text-center">🏠 Kotha Pasal</div>
